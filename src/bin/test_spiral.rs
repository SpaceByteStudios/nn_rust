...
fn spiral_data(amount : i32) -> Vec<DataPoint> {
    ...
    for i in 0..amount {
        ...
        data.push(DataPoint { input: Vector::new(vec![x, y]), exp_output: Vector::new(vec![0.0])});
        ...
        data.push(DataPoint { input: Vector::new(vec![x2, y2]), exp_output: Vector::new(vec![1.0])});
    }
